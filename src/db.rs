...
impl Db {
    ...
    fn migrate(&self) -> rusqlite::Result<()> {
        ...
        // Migration: add priority column only if it doesn't already exist.
        // Checking pragma_table_info keeps this idempotent without swallowing
        // a genuine ALTER TABLE failure (R7).
        let has_priority: i64 = self.conn.query_row(
            "SELECT COUNT(*) FROM pragma_table_info('issues') WHERE name = 'priority'",
            [],
            |row| row.get(0),
        )?;
        if has_priority == 0 {
            self.conn
                .execute("ALTER TABLE issues ADD COLUMN priority TEXT", [])?;
        }
    ...
    // allow: assembles one issue row's columns into JSON; the arguments mirror the
    // SELECT column list, so grouping them into a struct would only move the plumbing.
    ...
    fn require_issue(&self, id: i64, include_comments: bool) -> Result<Value> {
        self.full_issue(id, include_comments)?
            .ok_or(PitError::NotFound)
    }
    ...
    pub fn search_issues(&self, args: &Value) -> Result<Value> {
        ...
        // Per-issue match flags: (title, body, comments). Title and body are
        // queried with FTS5 column filters so each flag is reported accurately.
        let mut issue_matches: std::collections::HashMap<i64, (bool, bool, bool)> =
        ...
        let rows = stmt.query_map(params![format!("title : ({fts_query})")], |row| {
            row.get::<_, i64>(0)
        })?;
        for row in rows {
            issue_matches.entry(row?).or_insert((false, false, false)).0 = true;
        }
        ...
        let rows = stmt.query_map(params![format!("body : ({fts_query})")], |row| {
            row.get::<_, i64>(0)
        })?;
        for row in rows {
            issue_matches.entry(row?).or_insert((false, false, false)).1 = true;
        }
        ...
        for &issue_id in &all_ids {
            if let Some(issue) = self.full_issue(issue_id, false)? {
                if let Some(ref s) = req.status
                    && issue.get("status").and_then(|v| v.as_str()) != Some(s)
                {
                    continue;
                }
                if !req.labels.is_empty() {
                ...
                let (title_match, body_match, comment_match) = issue_matches[&issue_id];
                results.push(json!({
                    "issue": issue,
                    "matches": {
                        "title": title_match,
        ...
        // `total` is the full post-filter match count; `results` is capped at the limit.
    ...
    pub fn delete_issue(&self, args: &Value) -> Result<Value> {
        ...
        let changes = self
            .conn
            .execute("DELETE FROM issues WHERE id = ?1", params![req.id])?;
        if changes == 0 {
            return Err(PitError::NotFound);
        }
...
mod tests {
    ...
    fn create_issue_invalid_status() {
        let db = test_db();
        let err = db
            .create_issue(&json!({"title": "x", "status": "closed"}))
            .unwrap_err();
        match err {
    ...
    fn get_issue_includes_comments() {
        ...
        db.add_comment(&json!({"id": id, "body": "second"}))
            .unwrap();
    ...
    fn list_issues_filter_by_labels() {
        ...
        let result = db
            .list_issues(&json!({"labels": ["bug", "feature"]}))
            .unwrap();
        assert_eq!(result["total"], 1);
    ...
    fn list_issues_pagination() {
        ...
        let result = db
            .list_issues(&json!({"limit": 2, "offset": 0, "sort": "id", "order": "asc"}))
            .unwrap();
        assert_eq!(result["total"], 5);
        assert_eq!(result["issues"].as_array().unwrap().len(), 2);

        let result = db
            .list_issues(&json!({"limit": 2, "offset": 3, "sort": "id", "order": "asc"}))
            .unwrap();
        assert_eq!(result["issues"].as_array().unwrap().len(), 2);
    }
    ...
    fn list_issues_sort_by_created_asc() {
        ...
        create(&db, "second");
        let result = db
            .list_issues(&json!({"sort": "id", "order": "asc"}))
            .unwrap();
    ...
    fn update_issue_title() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "title": "new title"}))
            .unwrap();
    ...
    fn update_issue_body() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "body": "now has body"}))
            .unwrap();
    ...
    fn update_issue_labels_add() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "labels_add": ["p0"]}))
            .unwrap();
        let labels: Vec<&str> = updated["labels"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap())
    ...
    fn update_issue_labels_remove() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "labels_remove": ["bug"]}))
            .unwrap();
        let labels: Vec<&str> = updated["labels"]
            .as_array()
            .unwrap()
    ...
    fn update_issue_labels_set() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "labels_set": ["feature", "p1"]}))
            .unwrap();
        let labels: Vec<&str> = updated["labels"]
            .as_array()
            .unwrap()
    ...
    fn update_issue_updates_timestamp() {
        ...
        let updated = db
            .update_issue(&json!({"id": id, "title": "changed"}))
            .unwrap();
    ...
    fn add_comment_not_found() {
        let db = test_db();
        let err = db
            .add_comment(&json!({"id": 999, "body": "nope"}))
            .unwrap_err();
        match err {
            PitError::NotFound => {}
            other => panic!("expected NotFound, got {other:?}"),
    ...
    fn search_by_body() {
        ...
        create_with(
            &db,
            json!({"title": "issue", "body": "the frobnicator is broken"}),
        );
        create(&db, "unrelated");
    ...
    fn search_by_comment() {
        ...
        let id = issue["id"].as_i64().unwrap();
        db.add_comment(&json!({"id": id, "body": "the xylophone module crashes"}))
            .unwrap();
    ...
    fn search_with_status_filter() {
        ...
        db.update_issue(&json!({"id": cid, "status": "closed", "closed_reason": "completed"}))
            .unwrap();

        let result = db
            .search_issues(&json!({"query": "parser", "status": "open"}))
            .unwrap();
        assert_eq!(result["total"], 1);
    ...
    #[test]
    fn search_distinguishes_title_and_body_matches() {
        let db = test_db();
        create_with(
            &db,
            json!({"title": "alpha heading", "body": "beta gamma body"}),
        );

        // A term found only in the title reports title=true, body=false.
        let result = db.search_issues(&json!({"query": "alpha"})).unwrap();
        assert_eq!(result["results"][0]["matches"]["title"], true);
        assert_eq!(result["results"][0]["matches"]["body"], false);

        // A term found only in the body reports title=false, body=true.
        let result = db.search_issues(&json!({"query": "gamma"})).unwrap();
        assert_eq!(result["results"][0]["matches"]["title"], false);
        assert_eq!(result["results"][0]["matches"]["body"], true);
    }

    #[test]
    fn search_total_counts_all_matches_beyond_limit() {
        let db = test_db();
        for i in 0..10 {
            create(&db, &format!("searchable item {i}"));
        }
        let result = db
            .search_issues(&json!({"query": "searchable", "limit": 3}))
            .unwrap();
    ...
    fn search_no_results() {
        ...
        let result = db
            .search_issues(&json!({"query": "zzzznonexistent"}))
            .unwrap();
        assert_eq!(result["total"], 0);
    ...
    fn delete_cascades_comments_and_labels() {
        ...
        let id = issue["id"].as_i64().unwrap();
        db.add_comment(&json!({"id": id, "body": "a comment"}))
            .unwrap();
    ...
    fn create_issue_invalid_priority() {
        let db = test_db();
        let err = db
            .create_issue(&json!({"title": "x", "priority": "high"}))
            .unwrap_err();
        match err {
    ...
    fn update_issue_priority() {
        ...
        let id = issue["id"].as_i64().unwrap();
        let updated = db
            .update_issue(&json!({"id": id, "priority": "p1"}))
            .unwrap();
    ...
    fn link_and_get_issue_shows_links() {
        ...
        db.link_issues(&json!({"source_id": aid, "target_id": bid, "link_type": "blocks"}))
            .unwrap();
    ...
    fn link_self_rejected() {
        ...
        let aid = a["id"].as_i64().unwrap();
        let err = db
            .link_issues(&json!({"source_id": aid, "target_id": aid, "link_type": "blocks"}))
            .unwrap_err();
        match err {
    ...
    fn link_not_found() {
        ...
        let aid = a["id"].as_i64().unwrap();
        let err = db
            .link_issues(&json!({"source_id": aid, "target_id": 999, "link_type": "blocks"}))
            .unwrap_err();
        match err {
            PitError::NotFound => {}
            other => panic!("expected NotFound, got {other:?}"),
    ...
    fn unlink_issues_basic() {
        ...
        db.link_issues(&json!({"source_id": aid, "target_id": bid, "link_type": "relates_to"}))
            .unwrap();
        let result = db
            .unlink_issues(&json!({"source_id": aid, "target_id": bid, "link_type": "relates_to"}))
            .unwrap();
        assert_eq!(result["deleted"], true);
    ...
    fn unlink_not_found() {
        let db = test_db();
        let err = db
            .unlink_issues(&json!({"source_id": 1, "target_id": 2, "link_type": "blocks"}))
            .unwrap_err();
        match err {
            PitError::NotFound => {}
            other => panic!("expected NotFound, got {other:?}"),
    ...
    fn search_with_label_filter() {
        ...
        create_with(
            &db,
            json!({"title": "searchable feature", "labels": ["feature"]}),
        );

        let result = db
            .search_issues(&json!({"query": "searchable", "labels": ["bug"]}))
            .unwrap();
        assert_eq!(result["total"], 1);
