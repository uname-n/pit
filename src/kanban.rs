...
impl App {
        ...
        for col in &mut self.columns {
            ...
            match db.list_issues(&args) {
                Ok(v) => {
                    ...
                    let mut cards: Vec<IssueCard> = v
                        .get("issues")
                        .and_then(Value::as_array)
                        .map(|arr| arr.iter().map(parse_card).collect())
                        .unwrap_or_default();
                    // Stable sort floats higher-priority cards up while preserving
                    // the DB ordering (created/updated) within a priority band.
                    cards.sort_by_key(|c| priority_rank(c.priority.as_deref()));
                    col.issues = cards;
    ...
    fn move_item(&mut self, delta: i32) {
        ...
        // Clamp at the ends — moving past the top/bottom stays put (no wraparound).
        let next = (cur + delta).clamp(0, len - 1);
        col.state.select(Some(next as usize));
    }
}

/// Sort rank for a priority: p0 first, unprioritized last.
...
fn event_loop<B: Backend>(terminal: &mut Terminal<B>, app: &mut App, db: &Db) -> io::Result<()> {
    ...
    loop {
        ...
        if event::poll(timeout)? {
            match event::read()? {
                Event::Mouse(m) => handle_mouse(app, db, m),
                Event::Key(key) if handle_key(app, db, key) => return Ok(()),
                _ => {}
            }
        }
...
fn handle_key(app: &mut App, db: &Db, key: KeyEvent) -> bool {
    ...
    if app.detail.is_some() {
        ...
        handle_board_key(app, db, key)
    }
}

/// Keys handled while the detail pane is open. Returns `true` to quit.
fn handle_detail_key(app: &mut App, key: KeyEvent) -> bool {
    match key.code {
        ...
        KeyCode::Char('G') | KeyCode::End => app.detail_scroll = u16::MAX,
        _ => {}
    }
    false
}

/// Keys handled on the board (no detail pane). Returns `true` to quit.
fn handle_board_key(app: &mut App, db: &Db, key: KeyEvent) -> bool {
    match key.code {
        ...
        KeyCode::Left | KeyCode::Char('h') | KeyCode::BackTab => app.move_col(-1),
        KeyCode::Right | KeyCode::Char('l') | KeyCode::Tab => app.move_col(1),
...
fn render(f: &mut Frame, app: &mut App) {
    ...
    let mut outer = vec![Constraint::Length(1), Constraint::Length(1)];
...
fn render_detail(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    // rows, not logical lines, or the tail (comments) can't be reached.
...
/// matches the wrapping the detail Paragraph applies, so scrolling can reach the
/// true bottom. The scratch buffer height is bounded by an upper estimate.
fn wrapped_height(lines: &[Line<'static>], width: u16) -> u16 {
    let width = width.max(1);
    // Upper bound: each logical line needs at most (chars / width) + 1 rows.
    ...
    Paragraph::new(lines.to_vec())
        .wrap(Wrap { trim: false })
        .render(area, &mut buf);
    // Last row bearing any non-blank cell (+1 for a count).
    let mut rows = 1u16;
...
fn detail_lines(v: &Value, id: i64, status: &str, status_accent: Color) -> Vec<Line<'static>> {
    ...
    // Full title as a bold heading, colored to match its column's status accent.
    // The Paragraph wraps it, so long titles no longer clip against the border.
    ...
    for raw in body.lines() {
        ...
        if let Some(rest) = trimmed.strip_prefix("> ") {
            ...
            for mut s in parse_inline(rest) {
                s.style = s.style.fg(MUTED).add_modifier(Modifier::ITALIC);
                spans.push(s);
            }
            lines.push(Line::from(spans));
            continue;
