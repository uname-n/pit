...
fn main() {
    ...
    match args.first().map(String::as_str) {
        ...
        Some(other) => {
            eprintln!("pit: unknown subcommand: {other}");
            print_help();
            std::process::exit(2);
        }
...
fn print_help() {
    println!("pit — local issue tracker");
    println!();
    println!("usage:");
    println!("  pit              run as MCP server on stdio");
    println!("  pit kanban       live read-only kanban board (TUI)");
    println!("  pit help         show this message");
}
