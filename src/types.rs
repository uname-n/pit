...
fn validate_priority(p: &str) -> Result<()> {
    match p {
        "p0" | "p1" | "p2" | "p3" => Ok(()),
        _ => Err(PitError::InvalidParams(format!(
            "invalid priority: {p} (must be p0, p1, p2, or p3)"
        ))),
    }
}
...
impl CreateIssueRequest {
    pub fn parse(args: &serde_json::Value) -> Result<Self> {
        ...
        match req.status.as_str() {
            "open" | "in-progress" => {}
            _ => {
                return Err(PitError::InvalidParams(format!(
                    "invalid status: {}",
                    req.status
                )));
            }
...
fn validate_link_type(t: &str) -> Result<()> {
    match t {
        ...
        _ => Err(PitError::InvalidParams(format!(
            "invalid link_type: {t} (must be blocks, relates_to, or duplicates)"
        ))),
    }
}
...
impl LinkIssuesRequest {
    pub fn parse(args: &serde_json::Value) -> Result<Self> {
        ...
        if req.source_id == req.target_id {
            return Err(PitError::InvalidParams(
                "cannot link an issue to itself".into(),
            ));
        }
...
mod tests {
    ...
    fn create_issue_invalid_status() {
        let err =
            CreateIssueRequest::parse(&json!({"title": "x", "status": "closed"})).unwrap_err();
        match err {
    ...
    fn update_issue_invalid_closed_reason() {
        let err =
            UpdateIssueRequest::parse(&json!({"id": 1, "closed_reason": "nope"})).unwrap_err();
        match err {
    ...
    fn create_issue_invalid_priority() {
        let err =
            CreateIssueRequest::parse(&json!({"title": "x", "priority": "high"})).unwrap_err();
        match err {
    ...
    fn link_issues_valid() {
        let req = LinkIssuesRequest::parse(
            &json!({"source_id": 1, "target_id": 2, "link_type": "blocks"}),
        )
        .unwrap();
    ...
    fn link_issues_self_link_rejected() {
        let err = LinkIssuesRequest::parse(
    ...
    fn link_issues_invalid_type() {
        let err = LinkIssuesRequest::parse(
            &json!({"source_id": 1, "target_id": 2, "link_type": "depends_on"}),
        )
        .unwrap_err();
        match err {
    ...
    fn link_issues_all_types_valid() {
        for t in &["blocks", "relates_to", "duplicates"] {
            LinkIssuesRequest::parse(&json!({"source_id": 1, "target_id": 2, "link_type": t}))
                .unwrap();
        }
    }
    ...
    fn unlink_issues_valid() {
        let req = UnlinkIssuesRequest::parse(
            &json!({"source_id": 1, "target_id": 2, "link_type": "blocks"}),
        )
        .unwrap();
        assert_eq!(req.source_id, 1);
    ...
    fn search_issues_with_labels() {
        let req =
            SearchIssuesRequest::parse(&json!({"query": "bug", "labels": ["urgent"]})).unwrap();
